...
ticTacToe
	...
	.controller('TicTacToeController', function ($scope, $firebase) {
		...
		$scope.gameEnter = function(){
			$scope.playerId = playersRef.push({ name: $scope.name, symbol: "X", row0: 0, row1: 0, row2: 0, column0: 0, column1: 0, column2: 0, diagonal0: 0, diagonal1: 0}).name();
		...
		$scope.box = function(cellIndex){
				...
				for (props in $scope.players[$scope.playerId]){
						if($scope.players[$scope.playerId][props] == 3){
